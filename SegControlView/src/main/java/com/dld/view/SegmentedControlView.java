...
public class SegmentedControlView extends View implements ISegmentedControl
{
	...
	private static final int DEFAULT_DISABLED_OUTER_COLOR = Color.parseColor("#5012B7F5");
	 ...
	 */
	private int mDisabledOuterColor;
	...
	private void init(Context context, AttributeSet attrs)
	{
		...
		mTextSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, DEFAULT_TEXT_SIZE, context.getResources().getDisplayMetrics());
		mMode = ta.getInt(R.styleable.SegmentedControlView_segMode, Round);
		mDisabledOuterColor = ta.getColor(R.styleable.SegmentedControlView_segDisabledOuterColor, DEFAULT_DISABLED_OUTER_COLOR);
		mScrollEnable = ta.getBoolean(R.styleable.SegmentedControlView_segScrollEnable, true);

		CharSequence[] items = ta.getTextArray(R.styleable.SegmentedControlView_android_entries);
		if (items != null && items.length > 0)
		{
			List<SegmentedControlItem> ls = new ArrayList<>();
			for (CharSequence it : items)
			{
				ls.add(new SegmentedControlItem(it.toString()));
			}
			addItems(ls);
		}
	...
	public void removeOnSegItemClickListener()
	{
		this.listener = null;
	}
	...
	{
		if (position >= mSegmentedControlItems.size())
			throw new IllegalArgumentException("wrong position");

		mSegmentedControlItems.get(position).setEnabled(enabled);
		invalidate();
	}
	...
	{
		canvas.saveLayer(mStart, 0, mStart + mItemWidth, getHeight(), null);
		...
		for (int i = begin; i < end; i++)
		{
			...
			float x = start + mItemWidth / 2.0f - mTextPaint.measureText(getName(i)) / 2;
			float y = getHeight() / 2.0f - (mTextPaint.ascent() + mTextPaint.descent()) / 2;
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		if (action == MotionEvent.ACTION_DOWN)
		{
			...
			else if (isItemOutside(x, y))
			{
				...
				if (movePosition < mSegmentedControlItems.size() &&
						!mSegmentedControlItems.get(movePosition).getEnabled())
				{
					return mScrollEnable;
				}
		...
		else if (action == MotionEvent.ACTION_UP)
		{
			...
			else
			{
				...
				else
				{
					...
					if (newSelectedItem < mSegmentedControlItems.size() &&
							!mSegmentedControlItems.get(newSelectedItem).getEnabled())
					{
						newSelectedItem = mSelectedItem;
					}
	...
	{
		float r = mMode == Round ? mRadius : mHeight / 2.0f - mItemMarginTop;
		mPaint.setColor(mItemColor);

		//mPaint.setShader(new LinearGradient(0, 0, 0, mHeight, mItemColor, mItemColor + 200, Shader.TileMode.MIRROR));

		mRectF.set(mStart + mItemMarginLeft, mItemMarginTop, mStart + mItemWidth - mItemMarginLeft, getHeight() - mItemMarginTop);
	...
	private void drawBackground(Canvas canvas)
	{
		...
		float r = mMode == Round ? mRadius : mHeight / 2.0f;

		for (int i = 0; i < getCount(); i++)
		{
			if (mSegmentedControlItems.get(i).getEnabled())
				mPaint.setColor(mOuterColor);
			else
				mPaint.setColor(mDisabledOuterColor);

			mRectF.set(mItemMarginLeft + i * mItemWidth, 0, mItemMarginLeft + (i + 1) * mItemWidth, getHeight());

			if (i == 0)
			{
				canvas.save();
				canvas.clipRect(mRectF);
				canvas.drawRoundRect(new RectF(mRectF.left, mRectF.top, mRectF.right + r, mRectF.bottom), r, r, mPaint);
				canvas.restore();
			}
			else if (i == (getCount() - 1))
			{
				canvas.save();
				canvas.clipRect(mRectF);
				canvas.drawRoundRect(new RectF(mRectF.left - r, mRectF.top, mRectF.right, mRectF.bottom), r, r, mPaint);
				canvas.restore();
			}
			else
